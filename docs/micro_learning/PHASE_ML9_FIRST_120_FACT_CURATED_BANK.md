...
- Complete machine-prepared gate evidence now covers all 120 ML-9 candidates.
- Rights evidence records: 120.
- Curriculum mapping evidence records: 114 mapped facts.
- General-scope curriculum exemptions: 6 CSP reasoning facts.
- Pedagogy evidence records: 120.
- Accessibility review tracks: 120.
- Evidence is stored in 12 batch bundles under `content/micro_learning/evidence/`.
- Exact learner-text SHA-256 fingerprints and deterministic startup metrics are stored for every candidate.
- Rights evidence is bound to the exact source registry ID, URL, locator, edition/revision, MicroFact ID, and content version.
- Curriculum evidence is bound to blueprint `V.2024.04.24`, navigation registry `1.0.0`, and exact Domain/Competency mapping keys.
- General-scope facts carry no curriculum evidence, preserving the frozen ML-6 rule.
- Human rights, curriculum, pedagogy, and accessibility review states all remain `pending`.
- No human approval was generated by automation.
- No candidate was promoted to `validated` or `published`.
- Startup eligibility remains false for all 120 candidates.
- Repository validator: `tool/validate_ml9c_gate_evidence.dart`.
- CI workflow: `.github/workflows/ml9c_gate_evidence_validation.yml`.

ML-9C evidence manifest:

- `content/micro_learning/evidence/ml9c_gate_evidence_manifest_v1.json`

### ML-9D - Whole-Corpus ML-8 Pass

- The complete evidence-bound 120-fact corpus was revalidated using the frozen ML-8 deterministic comparator.
- Pairwise comparisons executed: 7,140.
- Exact duplicates: 0.
- Unresolved near-duplicate or contradiction review candidates: 0.
- Blocking corpus issues: 0.
- Human adjudications required: 0.
- Concentration signals: 0.
- ML-9C evidence binding was revalidated before the corpus pass.
- The dedicated ML-9D validator passed Dart analysis.
- The frozen ML-8 regression suite passed.
- No candidate fact content was changed by ML-9D.
- No human adjudication was generated by automation.
- All 120 facts remain in review, unpublished, and startup-ineligible pending ML-9E human review.

ML-9D artifacts:

- `content/micro_learning/review/ml9d_whole_corpus_ml8_report.json`
- `content/micro_learning/review/ml9d_whole_corpus_ml8_freeze.json`
- `tool/validate_ml9d_whole_corpus_ml8.dart`
- `.github/workflows/ml9d_whole_corpus_ml8_validation.yml`

### ML-9E - Human Review Preparation

- Human-review infrastructure is prepared for the complete 120-fact corpus.
- The review set is divided into 12 packets of 10 facts.
- Required review channels are technical, source, rights, curriculum when mapped, pedagogy, accessibility, and UI suitability.
- Every channel is initialized to pending.
- No reviewer identity, attestation, rationale, review date, or approval has been generated by automation.
- Six general-scope CSP reasoning facts correctly use curriculum review as not applicable.
- A passing channel requires explicit human identity, an allowed reviewer role, human attestation, rationale, valid review dates, and confirmation of every required check.
- A fact may be accepted only when every applicable review channel passes.
- A batch may be completed only when all 10 facts are accepted.
- ML-9E corpus completion requires all 120 facts accepted across all 12 completed packets.
- The normal validator checks review-package integrity.
- The `--require-complete` validator is intentionally expected to fail until genuine human review is complete.
- Candidate facts remain unchanged, in review, unpublished, and startup-ineligible.

ML-9E artifacts:

- `content/micro_learning/ml9e_human_review_packet_schema_v1.json`
- `content/micro_learning/human_review/ml9e_human_review_manifest_v1.json`
- `content/micro_learning/human_review/ml9e_batch_01_human_review.json` through `ml9e_batch_12_human_review.json`
- `content/micro_learning/review/ml9e_human_review_preparation_checkpoint.json`
