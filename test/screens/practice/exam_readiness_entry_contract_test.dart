...
void main() {
  group('Exam Readiness universal learner entry contract', () {
    ...
    test('Practice hub does not gate Exam Readiness on saved plan existence', () {
    ...
    test('shared Practice hub is used for light learner navigation', () {
      expect(
        bottomNavigation,
        contains('return const PracticeHubScreen();'),
      );
    });

    test('shared Practice hub is used for dark learner navigation', () {
      final occurrences =
          'return const PracticeHubScreen();'.allMatches(bottomNavigation);
      expect(occurrences.length, greaterThanOrEqualTo(2));
    });

    test('Practice remains a permanent learner bottom-navigation destination', () {
