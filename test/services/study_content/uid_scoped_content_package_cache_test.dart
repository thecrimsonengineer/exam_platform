...
import 'package:exam_platform/services/study_content/uid_scoped_content_package_cache.dart';

void main() {
  test('FR10B blocks protected content-package access while locked', () async {
    ...
    final package = _package('d01_c01');
  ...
  test('FR10B isolates package bytes by Firebase UID', () async {
    final store = _MemoryStore();
    final boundary = _FakeBoundary()
      ..authorize('user-a')
      ..authorize('user-b');

    final userA = UidScopedContentPackageCache(
      store: store,
      userId: 'user-a',
      accessBoundary: boundary,
    );
    final userB = UidScopedContentPackageCache(
      store: store,
      userId: 'user-b',
      accessBoundary: boundary,
    );

    final packageA = _package('d01_c01');
    final packageB = _package('d02_c01');
  ...
  test('FR10B verifies a content package before persisting it', () async {
    final store = _MemoryStore();
  ...
  test('FR10B rejects a package larger than the frozen cache budget', () async {
    final store = _MemoryStore();
    final boundary = _FakeBoundary()..authorize('user-a');
    final package = _package('d01_c01');
    final cache = UidScopedContentPackageCache(
  ...
  test(
    'FR10B evicts least-recently-used content by compressed bytes',
    () async {
      final store = _MemoryStore();
      final boundary = _FakeBoundary()..authorize('user-a');
      ...
      final packageA = _package('d01_c01');
      final packageB = _package('d02_c01');
      final packageC = _package('d03_c01');
  ...
  test(
    'FR10B invalid replacement preserves the previous verified entry',
    () async {
      final store = _MemoryStore();
      final boundary = _FakeBoundary()..authorize('user-a');
      final cache = UidScopedContentPackageCache(
      ...
      final original = _package('d01_c01');
      ...
      final invalidDescriptor = PublishedContentPackageDescriptor(
  ...
  test(
    'FR10B corrupted read-back restores the previous cache atomically',
    () async {
      final store = _MemoryStore();
      final boundary = _FakeBoundary()..authorize('user-a');
      final cache = UidScopedContentPackageCache(
      ...
      final original = _package('d01_c01');
      final replacement = _package('d01_c01', version: 2);
  ...
  test('FR10B default cache budget is exactly 512 KiB', () {
    expect(UidScopedContentPackageCache.defaultMaxCompressedBytes, 512 * 1024);
  });
}
...
({PublishedContentPackageDescriptor descriptor, List<int> bytes}) _package(
  ...
  final domainId = competencyId.substring(0, 3);
  final competencyNumber = int.parse(competencyId.substring(5));
  ...
  final envelope = <String, dynamic>{
    ...
    'content': <String, dynamic>{
      ...
      'title': 'Study content $competencyId',
      'status': 'published',
      'version': version,
...
class _MemoryStore implements ProtectedContentPackageCacheStore {
