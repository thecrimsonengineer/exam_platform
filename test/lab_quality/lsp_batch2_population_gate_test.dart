...
void main() {
  ...
  test('Batch 2 manifest binds exactly ten isolated LAB versions', () {
    ...
    expect(manifest.entries.map((entry) => entry.identityKey).toSet(), hasLength(10));
    expect(
      manifest.entries.every(
        (entry) => entry.technicalLabPath.startsWith('content/lab_population_batch2/'),
      ),
      isTrue,
    );
  });

  test('Batch 2 passes binding, strict H0.3 and DQG300 for all 50 Decisions', () {
    final manifest = _batch2Manifest();
    var decisionCount = 0;
    var optionCount = 0;
    ...
    for (final entry in manifest.entries) {
      ...
      if (!quality.isValid) {
        for (final item in quality.decisionResults.where((item) => !item.isPass)) {
          failures.add(
            entry.identityKey +
                ':' +
                item.nodeId +
  ...
  test('Batch 2 has no exact Decision or option-set duplicates against production', () {
    ...
    for (final entry in original.entries) {
      for (final decision in _decisions(_readObject(entry.technicalLabPath))) {
    ...
    for (final entry in batch2.entries) {
      for (final decision in _decisions(_readObject(entry.technicalLabPath))) {
        ...
        expect(batchPrompts.add(prompt), isTrue, reason: 'Duplicate Batch 2 prompt: ' + prompt);
        expect(
  ...
  test('Batch 2 admits all ten entries through the immutable publication chain', () async {
    final manifest = _batch2Manifest();
