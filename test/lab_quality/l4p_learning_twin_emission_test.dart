...
void main() {
  ...
  test(
    'L4P emits each newly persisted decision once with stable completion',
    () async {
      ...
      expect(
        sink.events.first.eventId,
        route.first.decisionHistory.first.eventId,
      );
      expect(
        sink.events.last.eventId,
        route.completed.decisionHistory.last.eventId,
      );
    },
  );

  test(
    'L4P retains recovery, confidence, competency and Mistake DNA evidence',
    () async {
      final route = await _runRecoverySequence();
      final sink = InMemoryLabLearningEvidenceSink();
  ...
  test('L4P repeated emission is idempotent by Decision Event ID', () async {
    ...
    expect(
      sink.events.single.eventId,
      route.first.decisionHistory.single.eventId,
    );
  });

  test(
    'L4P sink failure cannot roll back or mutate the persisted LAB story',
    () async {
      final route = await _runRecoverySequence();
      ...
      expect(result.failedEventIds, <String>[
        route.first.decisionHistory.single.eventId,
      ]);
  ...
  test(
    'L4P rejects rewritten committed history before emitting evidence',
    () async {
      final route = await _runRecoverySequence();
      ...
      expect(
        result.issues.any(
          (issue) => issue.contains('rewrote committed Decision Event'),
        ),
        isTrue,
      );
    },
  ...
  test(
    'L4P rejects cross-session or cross-version evidence transitions',
    () async {
      final route = await _runRecoverySequence();
  ...
  test(
    'L4P per-event builder rejects a substituted event with the same ID',
    () async {
      final route = await _runRecoverySequence();
  ...
  test(
    'L4P rejects appended history when session revision does not advance',
    () async {
      final route = await _runRecoverySequence();
  ...
  test(
    'L4P does not alter frozen retrospective completed-session semantics',
    () async {
      final route = await _runRecoverySequence();
  ...
  test(
    'L4P can catch up multiple append-only events after an evidence outage',
    () async {
      final route = await _runRecoverySequence();
      final sink = InMemoryLabLearningEvidenceSink();
