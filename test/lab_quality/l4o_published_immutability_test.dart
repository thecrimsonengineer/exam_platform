...
void main() {
  test('L4O accepts a valid immutable manual published snapshot', () async {
    ...
    expect(
      LabPackage.decode(stored!.publishedJson).metadata.lifecycle,
  ...
  test(
    'L4O rejects repository identity that disagrees with published JSON',
    () async {
      final repository = InMemoryLabPublishedRepository();
  ...
  test(
    'L4O rejects automated publication with an incomplete certificate set',
    () async {
      final repository = InMemoryLabPublishedRepository();
  ...
  test(
    'L4O rejects certificates after Decision Node content is changed',
    () async {
      final source = await _automatedVersion();
      ...
      firstOption['text'] =
          'Runtime-significant text changed after validation.';
  ...
  test(
    'L4O refuses revision creation from imported PUBLISHED-looking JSON',
    () {
  ...
  test(
    'L4O rejects revision creation after nested published root mutation',
    () async {
      final repository = InMemoryLabPublishedRepository();
  ...
  test(
    'L4O legitimate revision preserves the immutable original snapshot',
    () async {
      final repository = InMemoryLabPublishedRepository();
      final service = Lab1000StudioService(repository: repository);
      final draft = service.importJson(_draftSource());
