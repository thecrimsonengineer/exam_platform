...
void main() {
  ...
  for (var i = 0; i < 10; i++) {
    test('LAB-9 recovery convergence and authored endings ' + (i + 1).toString(), () async {
      final package = l3ReferenceDraftPackage();
      final lane = i % 5;
  ...
  for (var i = 0; i < 10; i++) {
    test('LAB-9 session interruption and resume ' + (i + 1).toString(), () async {
      final package = l3ReferenceDraftPackage();
  ...
  for (var i = 0; i < 10; i++) {
    test('LAB-9 debrief and Learning Twin evidence ' + (i + 1).toString(), () async {
      final package = l3ReferenceDraftPackage();
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        if (i == 0 || i == 1) {
          ...
          expect(
            learnerSource,
            isNot(contains('Lab1000StudioScreen')),
          );
          ...
          final first = await runL3WeakRoute(
            package,
            sessionId: 'golden_a',
          );
          final second = await runL3WeakRoute(
            package,
            sessionId: 'golden_b',
          );
