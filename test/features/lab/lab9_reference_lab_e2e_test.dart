...
void main() {
  for (var i = 0; i < 10; i++) {
    test('LAB-9 JSON Studio publish learner load ' + (i + 1).toString(), () async {
      ...
      expect(published.package.metadata.lifecycle, LabLifecycleStatus.published);
  ...
  for (var i = 0; i < 10; i++) {
    test('LAB-9 safe route ' + (i + 1).toString(), () async {
      final run = await safeReferenceRun(
        sessionId: 'safe_e2e_' + i.toString(),
      );
  ...
  for (var i = 0; i < 10; i++) {
    test('LAB-9 weak route ' + (i + 1).toString(), () async {
      final run = await weakReferenceRun(
        sessionId: 'weak_e2e_' + i.toString(),
      );
      expect(run.session.status, LabSessionStatus.completed);
  ...
  for (var i = 0; i < 10; i++) {
    test('LAB-9 critical route ' + (i + 1).toString(), () async {
      ...
      expect(run.session.decisionHistory.first.gateTriggered, 'permit_critical_event');
    });
  }

  for (var i = 0; i < 10; i++) {
    test('LAB-9 recovery and convergence route ' + (i + 1).toString(), () async {
      final run = await recoveryReferenceRun(
        sessionId: 'recovery_e2e_' + i.toString(),
      );
      expect(run.session.endingId, 'controlled_recovery');
      expect(run.session.decisionHistory[1].gateTriggered, 'emergency_convergence');
  ...
  for (var i = 0; i < 10; i++) {
    test('LAB-9 resume debrief Learning Twin evidence ' + (i + 1).toString(), () async {
  ...
  for (var i = 0; i < 10; i++) {
    test('LAB-9 deterministic golden replay and UI boundary ' + (i + 1).toString(), () async {
      if (i < 5) {
        final package = referenceLabPackage();
