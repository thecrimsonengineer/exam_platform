...
void main() {
  ...
  for (var i = 0; i < 10; i++) {
    test(
      ...
      () async {
        ...
        final item = bridge
            .buildEvidence(package: package, session: session)
            .single;
  ...
  for (var i = 0; i < 10; i++) {
    test(
      'LAB-7 failed and partial session rules ' + (i + 1).toString(),
      () async {
        final package = l3ReferenceDraftPackage();
        final session = i < 5
            ? await runL3Route(
  ...
  for (var i = 0; i < 10; i++) {
    test(
      'LAB-7 privacy and Story Engine isolation ' + (i + 1).toString(),
      () async {
        final package = l3ReferenceDraftPackage();
        final session = await runL3Route(
          package: package,
        ...
        expect(
          sink.events.single.toSanitizedJson().containsKey('userId'),
          isFalse,
        );
