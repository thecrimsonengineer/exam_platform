...
void main() {
  const inventoryPath =
      'assets/flashcards/production/d02/d02_c11/'
      'd02_c11_concept_inventory.v1.json';
  const sourcePath =
  ...
  test('D02 C11 candidates are unique and source-backed', () {
    ...
    expect(slugs, contains('return_on_investment'));
    expect(slugs, contains('benefit_cost_analysis'));
    expect(slugs, contains('acquisition_planning'));
    expect(slugs, contains('price_analysis'));
    expect(slugs, contains('cost_analysis'));
  ...
  test('D02 C11 preserves generic planning and resource concepts', () {
    final inventory = readObject(inventoryPath);
