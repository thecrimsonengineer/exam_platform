...
void main() {
  const inventoryPath =
      'assets/flashcards/production/d02/d02_c14/'
      'd02_c14_concept_inventory.v1.json';
  const sourcePath =
  ...
  test('D02 C14 candidates are unique and source-backed', () {
    ...
    expect(slugs, contains('arithmetic_mean'));
    expect(slugs, contains('confidence_interval'));
    expect(slugs, contains('sampling_representativeness'));
    expect(slugs, contains('pareto_analysis'));
  ...
  test('D02 C14 keeps earlier generic management concepts canonical', () {
    final inventory = readObject(inventoryPath);
