...
void main() {
  const inventoryPath =
      'assets/flashcards/production/d02/d02_c12/'
      'd02_c12_concept_inventory.v1.json';
  const sourcePath =
  ...
  test('D02 C12 candidates are unique and source-backed', () {
    ...
    expect(slugs, contains('theory_x_management'));
    expect(slugs, contains('theory_y_management'));
    expect(slugs, contains('transactional_leadership'));
    expect(slugs, contains('transformational_leadership'));
    expect(slugs, contains('progressive_discipline'));
  ...
  test('D02 C12 preserves generic safety leadership concepts', () {
    final inventory = readObject(inventoryPath);
