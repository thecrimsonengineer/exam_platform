...
void main() {
  ...
  testWidgets('review player reveals source and keeps rating buttons available', (
    tester,
  ) async {
    ...
    expect(find.byKey(const ValueKey('review-reveal-button')), findsOneWidget);
    ...
    expect(find.byKey(const ValueKey('review-got-it-button')), findsOneWidget);
    expect(find.textContaining('Source: NIOSH'), findsOneWidget);

    await tester.tap(
      find.byKey(ValueKey('flashcard-source-${card.id}')),
    );
    await tester.pumpAndSettle();
