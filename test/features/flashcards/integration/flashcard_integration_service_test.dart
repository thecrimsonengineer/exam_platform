...
void main() {
  ...
  FlashcardIntegrationService service({String learnerId = 'fc7-learner'}) {
  ...
  test(
    'Question completion resolves mapping and collects after persistence',
    () async {
      ...
      expect(result.status, FlashcardQuestionCompletionStatus.newlyCollected);
  ...
  test(
    'repeated mapped Question reinforces and same event is idempotent',
    () async {
      final integration = service();
      ...
      expect(first.status, FlashcardQuestionCompletionStatus.newlyCollected);
      expect(reinforced.status, FlashcardQuestionCompletionStatus.reinforced);
      expect(
        duplicate.status,
  ...
  test('unmapped Question is a no-op rather than a Quiz failure', () async {
    ...
    expect(
      await collectionRepository.loadAllOwnership(learnerId: 'fc7-learner'),
      isEmpty,
    );
  });

  test('aggregate mapping coverage identifies unmapped Questions', () async {
    final report = await service().questionMappingCoverage(const <int>[
      930001,
      930002,
      930003,
    ]);
  ...
  test('conflicting cross-package Question mapping fails closed', () async {
    ...
    packageRepository = MemoryFlashcardPackageRepository(
      seed: <FlashcardContentPackage>[referencePackage, conflicting],
    );
  ...
  test(
    'StudyContent placement validation uses real canonical hierarchy',
    () async {
      ...
      expect(
        invalid.issues.every((issue) => issue.level == 'competency'),
        isTrue,
      );
    },
  );
  ...
  test(
    'scoped review includes only due cards inside requested placement',
    () async {
      ...
      review.validate();
      await reviewRepository.save(learnerId: 'fc7-learner', state: review);
  ...
  test(
    'source and collection quality reports expose actionable health',
    () async {
  ...
  test(
    'Learning Twin summary is sanitized to counts and Domain IDs only',
    () async {
      final card = referencePackage.cards.first;
      final viewedAt = DateTime.utc(2026, 9, 18, 8);
      final now = DateTime.utc(2026, 9, 20, 8);
