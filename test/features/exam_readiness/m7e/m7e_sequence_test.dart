...
void main() {
  ...
  test('Sequence B excellent evidence reduces need for repair repetition', () {
    ...
    final weak = planner.generate(
      ...
      readinessProfiles: _stableM7eBaseline(
        overrideId: 'd03_c02',
        overrideProfile: m7dProfile(application: 0.3, gaps: [m7dGap()]),
      ),
    );
    ...
    expect(
      strong.blocks.any((b) => b.type == StudyPlanBlockType.repair),
      isFalse,
    );
  });
  ...
  test(
    'Sequence E high-confidence incorrect triggers calibration repair signal',
    () async {
      ...
      expect(
        result.misconceptionSignals.expand((signal) => signal.reasonCodes),
  ...
  test(
    'same baseline plus new evidence can produce justified next-day plan',
    () async {
      ...
      expect(after.blocks.any((block) => block.reasonCodes.isNotEmpty), isTrue);
    },
  );
}
