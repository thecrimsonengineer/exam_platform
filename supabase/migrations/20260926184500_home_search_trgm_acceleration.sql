create index if not exists published_study_search_index_normalized_trgm_idx
  on public.published_study_search_index
  using gin (normalized_text extensions.gin_trgm_ops);
...
with params as (
  ...
  where not exists (select 1 from strict_matches)
    ...
    and params.query_text OPERATOR(extensions.<%) idx.normalized_text
), scored as (
